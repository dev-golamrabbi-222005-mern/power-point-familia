...
export default function ManagerFinanceTab({ token, onNavigateToTickets, onError }: ManagerFinanceTabProps) {
  ...
  const fetchFinanceData = async () => {
    try {
      setLoading(true);
      const [membersRes, statsRes, recordsRes, depositsRes, expensesRes, billsRes, paymentsRes] = await Promise.all([
        ...
        fetch('/api/utilities', { headers: { Authorization: `Bearer ${token}` } }),
        fetch('/api/public/ledger', { headers: { Authorization: `Bearer ${token}` } }),
      ]);
      ...
      const approvedExpenses = expenses.filter(e => e.status === 'approved' && e.date.startsWith(currentMonth));
      const messMealCost = approvedExpenses.reduce((sum, e) => sum + e.totalCost, 0);
      setTotalMessMealCost(messMealCost);

      // 2. Meal Cash Deposited
      const approvedDeposits = deposits.filter(d => d.status === 'approved' && d.date.startsWith(currentMonth));
      const totalMealCashCollected = approvedDeposits.reduce((sum, d) => sum + d.amount, 0);
      setTotalRemainingMealCash(totalMealCashCollected - messMealCost);

      // 3. Fixed Payments
      const sharedBills: any[] = utilitiesData.bills || [];
      const totalFixedMessCost = sharedBills.filter(b => b.month === currentMonth).reduce((sum, b) => sum + b.totalAmount, 0);
      setTotalFixedPaymentAmount(totalFixedMessCost);

      // Member Count
      const memberCount = Math.max(users.filter(u => u.status === 'approved').length, 1);
      ...
      const rows: MemberFinanceRow[] = users.map((member) => {
        ...
        const fixedStatus = fixedPaid >= fixedShare && fixedShare > 0 ? 'paid' : fixedPaid > 0 ? 'partial' : 'unpaid';

        // Meal Cash Deposited
        const memberMealCash = deposits
          .filter(d => d.userId === member.id && d.status === 'approved' && !d.remarks?.includes('Fixed Expense'))
          .reduce((sum, d) => sum + d.amount, 0);

        // Meals Eaten
        const memberMeals = records
          .filter(r => r.userId === member.id && r.date.startsWith(currentMonth))
    ...
    try {
      ...
      const res = await fetch('/api/admin/change-requests', {
        ...
        body: JSON.stringify({
          ...
          oldValue: { name: editingMember.name, phone: editingMember.phone, status: editingMember.status },
      ...
      alert(data.message || 'Waiting for Admin approval for making significant change.');
    ...
    return (
      <div className="flex items-center justify-center min-h-96">
        <div className="flex flex-col items-center space-y-2">
          ...
          <p className="text-xs text-zinc-400">Loading Manager Finance Overview...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-zinc-100 dark:border-zinc-800">
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              Detailed list of all members with fixed costs, meal cash given, meals eaten & 7-day absence due rule
            </p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs md:text-sm border-collapse">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {memberRows.map((row) => (
                <tr key={row.userId} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <p className="font-bold text-zinc-900 dark:text-white">৳{row.fixedTotalPaid} / ৳{row.fixedTotalShare}</p>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl max-w-md w-full border border-zinc-200 dark:border-zinc-800 shadow-2xl p-5 space-y-4">
            ...
            <form onSubmit={handleSaveMemberEdit} className="space-y-4">
              ...
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs text-amber-400 font-medium">
                ⚠️ Edits made by Manager require Admin approval before taking effect.
              </div>
              ...
              >
                {editSubmitting ? 'Requesting Approval...' : 'Submit Edit Request to Admin'}
              </button>
            </form>
          </div>
        </div>
